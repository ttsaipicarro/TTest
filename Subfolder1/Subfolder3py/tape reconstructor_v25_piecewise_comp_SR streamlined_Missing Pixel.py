...
usefulfilename = \
'plume_1382542448_2013_10_23__8_34_unpickled.txt'
...
def windfunc (a):
    ...
    """
    allpixelheights = [0.401, 0.401+0.635,0.401+(0.635*2), 0.401+(0.635*3), 0.401+(0.635*4), 0.401+(0.635*5)] #For 6 pixels
    #No D or pixel 4
    pixelheights = [0.401, 0.401+0.635,0.401+(0.635*2),0.401+(0.635*4), 0.401+(0.635*5)] 
    #No B
    #pixelheights =[0.401,0.401+(0.635*2), 0.401+(0.635*3), 0.401+(0.635*4), 0.401+(0.635*5)]
    #No A
    #pixelheights = [0, 0.401+0.635,0.401+(0.635*2), 0.401+(0.635*3), 0.401+(0.635*4), 0.401+(0.635*5)] 
    #NoE
    #pixelheights = [0.401, 0.401+0.635,0.401+(0.635*2), 0.401+(0.635*3), 0.401+(0.635*5)]
    
    ha = 2.98
    w = [(i/ha)**a for i in allpixelheights]
...
#No pixel D
zarray = transpose(vstack([pixelheights, array([tapeAint, tapeBint, tapeCint, tapeEint, tapeFint])]))
#No pixel B
#zarray = transpose(vstack([pixelheights, array([tapeAint, tapeCint, tapeDint, tapeEint, tapeFint])]))
#No pixel A
#zarray = transpose(vstack([pixelheights, array([0, tapeBint, tapeCint, tapeDint, tapeEint, tapeFint])]))
#zarray = transpose(vstack([pixelheights, array([tapeAint, tapeBint, tapeCint, tapeDint, tapeFint])]))
####################################################################
####################################################################
####################################################################
...
lineoverheadratio = Fint/sum([Aint,Bint,Cint,Eint,Fint])
print "Line integral overhead ratio is (>0.2 is overheady): %f" %lineoverheadratio
if lineoverheadratio > 0.2:
    lineflag = -1
else:
    lineflag = 0
...
if plot3D == 'y':
    ...
    YCoords = pixelheights
    #PixelA replaced with ground
    #YCoords = pixelheights[1:] #For deleting pixelA
    yPad = 0.001
...
########################################################################
    """
    Making a pixel disappear
    """            
    pointsA = getPoints(tapeAdist, 0)
    pointsB = getPoints(tapeBdist, 1)
    pointsC = getPoints(tapeCdist, 2)
    #pointsD = getPoints(tapeDdist, 3)
    pointsE = getPoints(tapeEdist, 3)
    pointsF = getPoints(tapeFdist, 4)
    points = vstack([pointsA, pointsB, pointsC, pointsE, pointsF])
    #points = vstack([pointsB, pointsC, pointsD, pointsE, pointsF])
    ...
    if savePlot == 'y':
        if truncate == 'y':
            ...
            fn = filename[:len(filename)-14]+'_piecewise_figure_5px.png'
        fig.savefig(fn)
    plt.show()
